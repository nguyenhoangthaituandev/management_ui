...
const getNavItems = (userRole) => {
  let items = cloneArray(navItems).filter(item => item.roles && item.roles.includes(userRole));
  items.forEach(item => {
    item.pages = item.pages.filter(item => item.roles && item.roles.includes(userRole));
  });
